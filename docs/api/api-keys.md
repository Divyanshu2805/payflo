...
| `POST` | `/v1/merchants/api-keys/{id}/rotate` | — | `200` `ApiKeyCreateResponse` with a new `keySecret` | The old secret keeps working for 24 hours (`grace_period_expires_at`), so an integration can switch without downtime. `keyId` doesn't change. Rotating a revoked key fails with a `500` today ([known gaps](../gaps.md)). |
