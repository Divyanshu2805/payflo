...
| REST APIs | 7 endpoints — `POST /v1/auth/signup`; `POST`/`GET`/`DELETE`/`POST .../rotate` under `/v1/merchants/{merchantId}/api-keys`; `POST`/`GET .../{orderId}` under `/v1/orders` (see [APIs](api.md)) |
...
**Next up:** hashing for the signup password and API key secret, merchant login, and the rest of
the payment domain (list orders, payment lifecycle).
