...
**Next up:** hashing for the signup password and API key secret, merchant login, a way to move a
payment from `AUTHORIZING` to `AUTHORIZED` (a bank callback/webhook receiver, or a status-poll —
nothing currently does, so `capture` can't succeed yet, see [Known
gaps](gaps.md)), and the rest of the payment lifecycle (list orders,
refund).
