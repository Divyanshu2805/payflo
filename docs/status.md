...
| REST APIs | 6 endpoints — `POST /v1/auth/signup`; `POST`/`GET`/`DELETE`/`POST .../rotate` under `/v1/merchants/{merchantId}/api-keys`; `POST /v1/orders` (see [APIs](api.md)) |
...
the payment domain (list/get order, payment lifecycle).
