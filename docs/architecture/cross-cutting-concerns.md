...
Each service's tables are created and altered by Hibernate (`ddl-auto: update`). There is no migration tool, no version history and no rollback; a removed column is never dropped. See [schema conventions](../schema.md) and [known gaps](../gaps.md).
