...
How PayFlo separates merchants, authenticates callers at the gateway, confines card data to the vault, and signs webhooks is described in the [security model](docs/architecture/security-model.md). The rules every change must respect are in the [security guardrails](docs/practices.md), and the known open issues in [known gaps](docs/gaps.md).
